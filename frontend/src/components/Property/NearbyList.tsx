...
export const NearbyList: React.FC<{ properties: Property[], userLocation?: { lat: number, lon: number } | null, disableSorting?: boolean }> = ({ properties, userLocation, disableSorting = false }) => {
    // Recommendation Logic:
    // 1. Filter out invalid properties if any
    // 2. Sort:
    //    - If disableSorting is true: Use properties as-is (parent controlled)
    //    - If userLocation is enabled: Sort by Distance (Ascending)
    //    - Default: Sort by Views (Descending)
    // 3. Limit to 5 items

    const sorted = disableSorting ? properties : [...properties].sort((a, b) => {
        if (userLocation) {
    ...
    return (
        <div className="nearby-list" aria-live="polite">
            {top.map(p => {
                // Remove local distance calculation if already present
