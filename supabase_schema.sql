...
create policy "Users can update own users" on public.users for update using (auth.uid() = id);
...
create policy "Verified landlords/agents can create properties" on public.properties for insert with check (
  auth.uid() = owner_id AND exists (select 1 from public.users where id = auth.uid() and role in ('landlord', 'agent'))
);
...
create table public.applications (
  ...
  property_owner_id uuid not null references public.users(id), -- Denormalized for RLS efficiency
...
create policy "Admins see complaints" on public.complaints for select using (exists (select 1 from public.users where id = auth.uid() and role = 'admin'));
create policy "Users report complaints" on public.complaints for insert with check (auth.uid() = reporter_id);


// src/pages/Register.tsx 核心逻辑设计
const handleRegister = async () => {
  ...
  const { data, error } = await supabase.auth.signUp({
    ...
    options: {
      data: {
        full_name: formData.fullName, // 必填，触发器需要这个
        role: formData.role           // 必填，触发器需要这个
      }
    }
  });
...
// src/context/AuthContext.tsx 核心逻辑设计
useEffect(() => {
  // 初始化获取 Session
  supabase.auth.getSession().then(({ data: { session } }) => {
    setSession(session);
    if (session) fetchUserProfile(session.user.id); // 去 public.users 查角色
  });

废弃本地状态管理，改为监听 Supabase Session。
  // 监听变化 (登录/登出)
  const { data: { subscription } } = supabase.auth.onAuthStateChange((_event, session) => {
    setSession(session);
    if (session) fetchUserProfile(session.user.id);
    else setUser(null); // 清空
  });
