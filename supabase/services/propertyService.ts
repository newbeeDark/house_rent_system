...
export class PropertyService {
  async searchProperties(filters: z.infer<typeof searchSchema>) {
    ...
    if (area_min) query = query.gte('area', area_min)
    if (area_max) query = query.lte('area', area_max)
  ...
  async createProperty(landlordId: string, data: z.infer<typeof propertySchema>) {
  ...
  async updateProperty(propertyId: string, landlordId: string, updates: z.infer<typeof updatePropertySchema>) {
  ...
  async getUserProperties(landlordId: string) {
    const { data, error } = await supabase
      .from('properties')
      .select(`
        *,
